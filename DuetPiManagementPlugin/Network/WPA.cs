    ...
    public static class WPA
    {
        ...
        public static async Task<string> Start()
        {
            ...
            if (!await Command.ExecQuery("/usr/bin/systemctl", "is-enabled -q wpa_supplicant.service"))
            {
                ...
                builder.AppendLine(await Command.Execute("/usr/bin/systemctl", "enable wpa_supplicant.service"));
            }
        ...
        public static async Task<string> Stop()
        {
            ...
            if (await Command.ExecQuery("/usr/bin/systemctl", "is-enabled -q wpa_supplicant.service"))
            {
                ...
                builder.AppendLine(await Command.Execute("/usr/bin/systemctl", "disable wpa_supplicant.service"));
            }
            return builder.ToString().Trim();
        }
        ...
        public static async Task<List<string>> GetSSIDs()
        {
            ...
            {
                using FileStream configStream = new FileStream("/etc/wpa_supplicant/wpa_supplicant.conf", FileMode.Open, FileAccess.Read);
        ...
        public static async Task<Message> Report()
        {
            ...
            if (ssids.Count > 0)
            {
                ...
                foreach (string ssid in await GetSSIDs())
                {
        ...
        public static async Task<Message> UpdateSSID(string ssid, string psk, string countryCode = null)
        {
            ...
            if (!File.Exists("/etc/wpa_supplicant/wpa_supplicant.conf"))
            {
                ...
                using FileStream configTemplateStream = new FileStream("/etc/wpa_supplicant/wpa_supplicant.conf", FileMode.Create, FileAccess.Write);
                using StreamWriter writer = new StreamWriter(configTemplateStream);
            ...
            using (FileStream configStream = new FileStream("/etc/wpa_supplicant/wpa_supplicant.conf", FileMode.Open, FileAccess.ReadWrite))
            {
                ...
                {
                    using StreamWriter countryCodeWriter = new StreamWriter(configStream);
